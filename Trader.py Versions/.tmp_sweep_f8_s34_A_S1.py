...
class Trader:
    ...
    def _osmium_kalman(self, od, pos, limit, buy_cap, sell_cap, timestamp, data):
        ...
        sweep_enabled = abs(obi) >= 0.5 and (timestamp - last_sweep >= 1000)
        sweep_fired = False
    ...
    def _pepper(self, od, pos, limit, buy_cap, sell_cap, timestamp, data, osmium_pos=0):
        ...
        fast_alpha = 2.0 / (8.0 + 1.0)
        slow_alpha = 2.0 / (34.0 + 1.0)
        ...
        raw_kalman_gain = pepper_var / max(1e-6, (pepper_var + measurement_var))
