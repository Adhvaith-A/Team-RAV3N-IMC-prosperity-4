...
class Trader:
    ...
    def _pepper(self, od, pos, limit, buy_cap, sell_cap, timestamp, data, osmium_pos=0):
        ...
        if abs(alpha_signal) > 1.5 * sigma:
            hedge_ratio = -0.3 * (osmium_pos / float(max(1, limit)))
        ...
        spread = self._clamp(1.0 + min(0.45, math.sqrt(pepper_var) * 0.05), 1.0, 1.5)
        ...
        taker_threshold = self._clamp(1.5 + (1.0 - kalman_gain) * 0.25, 1.5, 1.8)
